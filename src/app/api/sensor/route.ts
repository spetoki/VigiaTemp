...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!macAddress || typeof temperature !== 'number') {
      return NextResponse.json({ message: 'Endereço MAC e temperatura são obrigatórios.' }, { status: 400 });
    }

    // Chama a função de serviço para encontrar o sensor pelo MAC e atualizar seus dados
    const updatedSensor = await updateSensorDataFromDevice(macAddress, temperature);

    if (!updatedSensor) {
        return NextResponse.json({ message: `Sensor com MAC ${macAddress} não encontrado. Cadastre o sensor primeiro.` }, { status: 404 });
    }
