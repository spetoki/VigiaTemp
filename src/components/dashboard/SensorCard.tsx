...
export default function SensorCard({ sensor }: SensorCardProps) {
  ...
  const statusConfig: Record<SensorStatus, { icon: React.ElementType; colorClass: string; label: string }> = {
    ...
    critical: { icon: AlertTriangle, colorClass: 'text-red-600', label: t('sensorCard.label.critical', 'Crítico') },
  };
      ...
      <CardHeader className="pb-2">
        <div className="flex justify-between items-start">
          ...
          <Badge variant={status === 'critical' ? 'destructive' : status === 'warning' ? 'default' : 'secondary'} 
                 className={cn(status === 'warning' && 'bg-yellow-500 text-white', status === 'critical' && 'bg-red-600 text-white')}>
            <CurrentStatusIcon className={cn("mr-1 h-4 w-4", status === 'normal' ? 'text-green-600' : 'text-white')} />
